...
{{
  ...
  "behavior": "主体行为自由描述，越具体越好。例如：奔跑追猫、跃起变身、接抱安抚、换装伪装、对话反应、远去吐槽、静止自省",
  "pace": "节奏标签，只能选以下之一：爆发/快/正常/慢/静止",
  "emotion": "整体情绪",
  "emotion_intensity": 3.5,
  "primary_subject": "画面主体，如：云琛、黑猫、小六、剪影/背影",
  "dialogue": "关键台词",
    ...
    def _parse_segments(self, data: Dict) -> List[Segment]:
        ...
        for seg in data.get("segments", []):
            segments.append(Segment(
                ...
                pace=seg.get("pace", ""),
                emotion_intensity=float(seg.get("emotion_intensity", 0.0) or 0.0),
                primary_subject=seg.get("primary_subject", ""),
